...
import {
  ...
  CustomTableAction,
  CustomType,
} from '@shared/components/custom-table/custom-table.constants';
...
export class CustomTableComponent implements OnChanges, AfterViewInit {
  ...
  @Input() pageSize = 10;
  @Input() mainActions: CustomTableAction[] = [];

  @Output() actionCallback: EventEmitter<CustomCellContext> = new EventEmitter<CustomCellContext>(true);
  ...
  customTableLoading = CUSTOM_CELL_LOADING;
  ...
  callActionCallback(callback: string, i: number) {
    const context: CustomCellContext = {callback: callback, uid: this._getElementUid(i)};
  ...
  hideHeaderRow(): boolean {
    if (this.displayColumns) {
      return this.displayColumns.filter((d: CustomColumn) => d.hidden).length === this.displayColumns.length;
    }
    return false;
  }
