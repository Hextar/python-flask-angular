...
import {
  ISTELLA_CELL_LOADING,
  IstellaColumn,
  IstellaContext,
  IstellaTableAction, IstellaType,
  SENTIMENT_LABELS
} from '@shared/components/istella-table/istella-table.constants';

@Component({
  selector: 'app-istella-table',
  templateUrl: './istella-table.component.html',
  styleUrls: ['./istella-table.component.scss'],
  changeDetection: ChangeDetectionStrategy.OnPush
})
export class IstellaTableComponent implements OnChanges, AfterViewInit {
  ...
  @Input() pageSize = 10;
  @Input() mainActions: IstellaTableAction[] = [];

  @Output() actionCallback: EventEmitter<IstellaContext> = new EventEmitter<IstellaContext>(true);
  ...
  cellType = IstellaType;
  hoverCell = false;

  istellaTableLoading = ISTELLA_CELL_LOADING;
  ...
  ngOnChanges(changes: SimpleChanges) {
    ...
    if (dataSourceSC) {
     ...
     /* const totalItems: SimpleChange = changes.totalItems;
      if (totalItems) {
        const tpv = totalItems.previousValue ? totalItems.previousValue : null;
        const tcv = totalItems.currentValue ? totalItems.currentValue : null;
        if (this.dataSource && this.dataSource.paginator) {
          if (!!tcv && (tcv !== tpv || !this.dataSource.paginator.length)) {
            this.setPaginator();
          }
        }
      }*/
  ...
  callActionCallback(callback: string, i: number) {
    const context: IstellaContext = {callback: callback, uid: this._getElementUid(i)};
  ...
  hideHeaderRow(): boolean {
    if (this.displayColumns) {
      return this.displayColumns.filter((d: IstellaColumn) => d.hidden).length === this.displayColumns.length;
    }
    return false;
  }
  ...
  updatePageManually(index: any) {
    if (typeof index === 'number' && this.dataSource && this.dataSource.paginator) {
